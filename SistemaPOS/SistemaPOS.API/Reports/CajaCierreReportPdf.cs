...
{
    public class CajaCierreReportPdf : IDocument
    {
        private readonly CajaCierreReportData _d;
        public CajaCierreReportPdf(CajaCierreReportData data) => _d = data;
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                {
                    col.Item().Text($"Informe de Cierre de Caja #{_d.CajaId}").FontSize(18).SemiBold();
                    col.Item().Text($"Sede: {_d.SedeId} | Apertura: {_d.FechaApertura:yyyy-MM-dd HH:mm} | Cierre: {_d.FechaCierre:yyyy-MM-dd HH:mm}")
        ...
        void SummaryCard(IContainer c)
        {
            ...
            {
                col.Spacing(4);
                col.Item().Text($"Monto inicial: {_d.MontoInicial:n0}");
                ...
                col.Item().Text($"Monto final: {_d.MontoFinal:n0}").SemiBold();
            });
        }
        ...
        void VentasTable(IContainer c)
        {
            c.Table(t =>
            {
                ...
                foreach (var v in _d.Ventas)
                {
                    ...
                    t.Cell().Text(v.FechaVenta.ToString("HH:mm"));
